...
void CScanData::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MaxChars(pDX, m_strBarcData, 64);
	...
	DDV_MaxChars(pDX, m_strSerialNoData, 15);
	// C2K modem
...
bool CScanData::CheckScanData(const char *pStrHeader, bool bCheckHeader, const char *pInData, E_ScanData_Type dataType)
{
    ...
    {
    case Barcode:
        if (dataLength >= 65 || dataLength <= 0 )
        {
            sprintf(pScanMsg, "Barcode = \"%s\" length must be [1,64]", pInData);
            SetDlgItemText(IDC_SCAN_MSG, pScanMsg);           
            return false;
        }
        else
    ...
    case SerialNo:
		...
		else
		{
			...
			{
				sprintf(pScanMsg, "%s = \"%s\" length must not be longer than 15!", m_pstrScanItem, pInData);
				SetDlgItemText(IDC_SCAN_MSG, pScanMsg);
				return false;
			}
		}
...
void CScanData::RemoveWindow()
{
    ...
    CRect idtorDlgRect;  //indicator button rect
    ...
    g_pMainDlg->GetIndicatorPos(idtorDlgRect); 
    ...
    GetDlgItem(IDC_BT_DATA)->GetWindowRect(&editRect_2);
    ...
    int iEditGap = editRect_2.top - editRect.bottom + 10;
    ...
    {
        heightOffset = i*(iEditGap+iEditHeight);
        ...
        GetDlgItem(g_sCtrlIDInfo[i].staticID)->MoveWindow(iStaticX, iStaticY+heightOffset, iStaticWidth, iStaticHeigth);
        GetDlgItem(g_sCtrlIDInfo[i].editID)->MoveWindow(iEditX, iEditY+heightOffset, iEditWidth, iEditHeight);
        GetDlgItem(g_sCtrlIDInfo[i].headerID)->MoveWindow(iHeaderX, iHeaderY+heightOffset, iHeaderWidth, iHeaderHeight);
    }
    
    heightOffset = i*(iEditGap+iEditHeight);
    GetDlgItem(IDC_SCAN_MSG)->MoveWindow(msgEditRect.left, iEditY+heightOffset, msgEditRect.Width(), msgEditRect.Height());
    
    heightOffset = heightOffset + msgEditRect.Height();
    GetDlgItem(IDC_BTN_SCAN_CANCEL)->MoveWindow(cancelBtnRect.left, iEditY+heightOffset, cancelBtnRect.Width(), cancelBtnRect.Height());
    GetDlgItem(IDC_BTN_SCAN_OK)->MoveWindow(okBtnRect.left, iEditY+heightOffset, okBtnRect.Width(), okBtnRect.Height());
    
    
    int iWidth = dlgRect.Width();
    int iHeight = iEditHeight * 6 + heightOffset;
    
    int x = idtorDlgRect.left - (dlgRect.Width() - idtorDlgRect.Width()) / 2 ;
    int y = idtorDlgRect.top - iHeight;
