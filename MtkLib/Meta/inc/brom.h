...
// CUSTOMER1_ONLY
    ...
    ,MAUI_MODE_LOCAL      // Tier-1 SW local mode
#endif
...
#if defined(CUSTOMER1_ONLY)
//------------------------------------------------------------------------------
// boot LOCAL mode (Tier-1 project)
//------------------------------------------------------------------------------
typedef struct {
...
int  __stdcall Boot_LOCAL(
		HANDLE hCOM,
		const BOOT_LOCAL_ARG  *p_arg,
		BOOT_RESULT  *p_result,
		int *p_bootstop);
...
typedef struct {
    ...
    // to disable long-press pwkey setting
    _BOOL                               m_disable_long_press_power_key;
...
typedef struct{
    ...
    unsigned int        customize_pid_vid_en;
    unsigned int        psram_vid_read_dis;
