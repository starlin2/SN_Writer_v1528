...
#define	COM_NO_TIMEOUT			0
    ...
    typedef struct DA_REPORT_T {
        ...
        unsigned int        m_efuse_backup_data_1;
        unsigned int        m_efuse_backup_data_2;
        unsigned int        m_copy_of_efuse_backup_data_1;
        unsigned int        m_copy_of_efuse_backup_data_2;
...
typedef int (__stdcall *CALLBACK_PRE_FORMAT_PROGRESS)(unsigned char finished_percentage, void *usr_arg);
    ...
    typedef	struct {
    	...
    	// to enable DA log from target via USB
    	_BOOL				m_enable_da_logging_channel;
...
// CUSTOMER1_ONLY
#if defined(CUSTOMER1_ONLY)
    //------------------------------------------------------------------------------
    // Enable WatchDog Timeout and force reset to Local Mode (Tier-1 project)
    //------------------------------------------------------------------------------
    extern int __stdcall FlashTool_EnableWatchDogTimeoutAndResetToLocalMode(
        FLASHTOOL_API_HANDLE_T  ft_handle, unsigned int ms_timeout_interval);
#endif
// End Of CUSTOMER1_ONLY
...
// MTK_INTERNAL
    //------------------------------------------------------------------------------
    // Bootloader shared data retrival
    //------------------------------------------------------------------------------
    ...
    //   type          [IN]  record type
    //   data_buf      [OUT] buffer to hold record data read from target
    //   data_buf_len  [IN]  length of data_buf
    //                 [OUT] length of record data read from target
    //
    //  [Tier01-BLSharedData]
    extern int __stdcall FlashTool_ReadBootloaderSharedData(
        FLASHTOOL_API_HANDLE_T ft_handle,
        unsigned int type,
...
// MTK_INTERNAL
    //------------------------------------------------------------------------------
    // Internal Raw Write API
    //------------------------------------------------------------------------------
//#ifdef __MTK_INTERAL_DEV_LEVEL0__


    typedef enum
    {
        NandProgMode_PAGE_ONLY = 0,               // page only
        NandProgMode_PAGE_SPARE = 1,              // page+spare (ECC is not update): program PC raw data all.
        NandProgMode_PAGE_SPARE_ECC_UPDATE = 2    // page+spare (ECC update): not program ECC area from PC side.
    } FlashProgrammingMode;


    typedef struct
    {
        WriteFlashMemoryParameter  m_basic_write_para;
        FlashProgrammingMode  m_flash_prog_mode;
    } WriteFlashMemoryParameter_Internal;

    // [RawWrite_Ex]
    extern int __stdcall FlashTool_WriteFlashMemory_Internal(
        FLASHTOOL_API_HANDLE_T ft_handle,
        const WriteFlashMemoryParameter_Internal *parameter);
//#endif
// End Of MTK_INTERNAL
    ...
    typedef struct{
        ...
        unsigned int number;
        unsigned int size;
    }Mount_Flash_Block_Info;

    typedef struct{
        unsigned int            flash_total_size;
        unsigned int            flash_sector_count;
        Mount_Flash_Block_Info  blockInfo[8];
    }Mount_Flash_Info;

    typedef struct{
        Mount_Flash_Info    flash1;
        Mount_Flash_Info    flash2;
    }MountFlashLayout_T;

    extern int __stdcall FlashTool_MountFlashLayout(FLASHTOOL_API_HANDLE_T  ft_handle, MountFlashLayout_T *p_mount_flash_layout);
...
#define MEMORYTEST_STOP		9876
    ...
    // Log Enhanced
    //------------------------------------------------------------------------------
    extern int __stdcall FlashTool_EnableLoggingChannel(FLASHTOOL_API_HANDLE_T  ft_handle);
