...
const char *g_strToolVersion = "SN Writer Tool Version : V1.1528.00";
...
void CSNWriterDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX,IDC_MAINUI_TEMP2,m_MainUIT2);
	//}}AFX_DATA_MAP
...
BOOL CSNWriterDlg::OnInitDialog()
{
   ...
   // ASSERT(IDM_ABOUTBOX < 0xF000);
    ...
    m_font.CreatePointFont(430,"");

    //m_hSNMenu = LoadMenu(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDR_SN_MENU));
    ::SetMenu(this->GetSafeHwnd(),m_hSNMenu);
    
    m_hBitmapFail = ::LoadBitmap(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_BITMAP_FAIL));   //IDB_BITMAP1 ÎªBITMAP×ÊÔ´
...
void CSNWriterDlg::MenuItemInit()
{
    ...
    {
        m_SNMenu.GetSubMenu(1)->GetSubMenu(0)->CheckMenuItem(IDR_META_MODE, MF_CHECKED | MF_BYCOMMAND);
        m_SNMenu.GetSubMenu(1)->GetSubMenu(0)->CheckMenuItem(IDR_ATCMD_MODE, MF_UNCHECKED | MF_BYCOMMAND);
    }
    else if (g_sMetaComm.eOperatorMode == AT_MODE)
    {
        m_SNMenu.GetSubMenu(1)->GetSubMenu(0)->CheckMenuItem(IDR_ATCMD_MODE, MF_CHECKED | MF_BYCOMMAND);
        m_SNMenu.GetSubMenu(1)->GetSubMenu(0)->CheckMenuItem(IDR_META_MODE, MF_UNCHECKED | MF_BYCOMMAND);
    }
    ...
    {
        m_SNMenu.GetSubMenu(1)->GetSubMenu(5)->CheckMenuItem(IDR_OPTION_CHECK_CAL_FLAG, MF_CHECKED | MF_BYCOMMAND);
    }
    
    if (g_sMetaComm.bCheckFtFlag)
    {
        m_SNMenu.GetSubMenu(1)->GetSubMenu(5)->CheckMenuItem(IDR_OPTION_CHECK_FT_FLAG, MF_CHECKED | MF_BYCOMMAND);
    }
    ...
    {
		m_SNMenu.GetSubMenu(2)->EnableMenuItem(0, MF_GRAYED | MF_DISABLED | MF_BYPOSITION);
		m_SNMenu.GetSubMenu(2)->EnableMenuItem(2, MF_GRAYED | MF_DISABLED | MF_BYPOSITION);
    }
	else
	{
    ...
    {
        m_SNMenu.GetSubMenu(3)->CheckMenuItem(0, MF_CHECKED | MF_BYPOSITION);  
        m_SNMenu.GetSubMenu(3)->CheckMenuItem(1, MF_UNCHECKED | MF_BYPOSITION); 
    ...
    {
        m_SNMenu.GetSubMenu(3)->CheckMenuItem(1, MF_CHECKED | MF_BYPOSITION);  
        m_SNMenu.GetSubMenu(3)->CheckMenuItem(0, MF_UNCHECKED | MF_BYPOSITION); 
    }
}
...
{
    (CWnd*)GetDlgItem(IDC_COMBO_COMPORT)->EnableWindow(TRUE);   
    if (g_LoginIdentify == ENGINEER)
    {
        (CWnd*)GetDlgItem(IDC_BTN_SYSCONFIG)->EnableWindow(TRUE);
        (CWnd*)GetDlgItem(IDC_COMBO_TARGET_TYPE)->EnableWindow(TRUE);
		((CWnd*)GetDlgItem(IDC_MAINUI_TEMP1))->ShowWindow(false);
        ((CWnd*)GetDlgItem(IDC_MAINUI_TEMP2))->ShowWindow(false);
...
//		ShowTemperatureOnUI();
    ...
    {
        (CWnd*)GetDlgItem(IDC_BTN_SYSCONFIG)->EnableWindow(FALSE);
        (CWnd*)GetDlgItem(IDC_COMBO_TARGET_TYPE)->EnableWindow(FALSE);
		((CWnd*)GetDlgItem(IDC_MAINUI_TEMP1))->ShowWindow(false);
        ((CWnd*)GetDlgItem(IDC_MAINUI_TEMP2))->ShowWindow(false);
...
void CSNWriterDlg::DisableUIItem()
{
    (CWnd*)GetDlgItem(IDC_BTN_SYSCONFIG)->EnableWindow(FALSE);
    (CWnd*)GetDlgItem(IDC_COMBO_TARGET_TYPE)->EnableWindow(FALSE);
    (CWnd*)GetDlgItem(IDC_COMBO_COMPORT)->EnableWindow(FALSE);
    m_SNMenu.EnableMenuItem( 0, MF_GRAYED | MF_DISABLED | MF_BYPOSITION ); 
    m_SNMenu.EnableMenuItem( 1, MF_GRAYED | MF_DISABLED | MF_BYPOSITION ); 
    m_SNMenu.EnableMenuItem( 2, MF_GRAYED | MF_DISABLED | MF_BYPOSITION );
...
void CSNWriterDlg::PopupMsgBox(const char* strMsg, const char* strTilte, UINT uType)
{
...
void CSNWriterDlg::SetUIItem()
{
	...
	m_cTargetTypeBox.InsertString(C2K, "C2k Phone");
    m_cTargetTypeBox.SetCurSel(g_sMetaComm.eTargetType);   
}

void CSNWriterDlg::DebugOn()
{
...
{
  ((CWnd*)GetDlgItem(IDC_MAINUI_TEMP1))->ShowWindow(true);
  ((CWnd*)GetDlgItem(IDC_MAINUI_TEMP2))->ShowWindow(true);

   //m_MainUIT1.Format("%0.1f",g_sMetaComm.sIMEIOption.Temp_Option.Temperature);
   m_MainUIT1.Format("%s",g_sMetaComm.sIMEIOption.Temp_Option.strTemperatureData);
   m_MainUIT2.Format("%c%c", 0xA1, 0xE6); 

   //m_font.CreatePointFont(430,"");
   ((CWnd*)GetDlgItem(IDC_MAINUI_TEMP1))->SetFont(&m_font);
   ((CWnd*)GetDlgItem(IDC_MAINUI_TEMP1))->SetWindowText(m_MainUIT1);

   ((CWnd*)GetDlgItem(IDC_MAINUI_TEMP2))->SetFont(&m_font);  
   ((CWnd*)GetDlgItem(IDC_MAINUI_TEMP2))->SetWindowText(m_MainUIT2);
    //m_font.DeleteObject(); 
}
...
void CSNWriterDlg::OnSelchangeComboTargetType() 
{
	...
	m_SNMenu.GetSubMenu(2)->EnableMenuItem(0, MF_ENABLED | MF_BYPOSITION);
	m_SNMenu.GetSubMenu(2)->EnableMenuItem(2, MF_ENABLED | MF_BYPOSITION);
    ...
    else if (g_sMetaComm.eTargetType != FEATURE_PHONE && g_sMetaComm.eTargetType != TABLET_WIFI_ONLY) //Feature phone 6276M & 6280 traditional dongle also need to enable USB SwitchTool
    {
        g_sMetaComm.bSwithTool = false;
	...
	{
		m_SNMenu.GetSubMenu(2)->EnableMenuItem(0, MF_GRAYED | MF_BYPOSITION);
		m_SNMenu.GetSubMenu(2)->EnableMenuItem(2, MF_GRAYED | MF_BYPOSITION);
...
void CSNWriterDlg::OnAtcmdMode() 
{
    ...
    m_SNMenu.GetSubMenu(1)->GetSubMenu(0)->CheckMenuItem(0, MF_UNCHECKED | MF_BYPOSITION);
    m_SNMenu.GetSubMenu(1)->GetSubMenu(0)->CheckMenuItem(1, MF_CHECKED | MF_BYPOSITION);
    */
}
...
void CSNWriterDlg::OnOptionCheckCalFlag() 
{
    ...
    if(g_sMetaComm.bCheckCalFlag)
    {
        ...
        m_SNMenu.GetSubMenu(1)->GetSubMenu(5)->CheckMenuItem(IDR_OPTION_CHECK_CAL_FLAG, MF_UNCHECKED | MF_BYCOMMAND);
    }
    else
    {
...
void CSNWriterDlg::OnOptionCheckFtFlag() 
{
    ...
    if(g_sMetaComm.bCheckFtFlag)
    {
        ...
        m_SNMenu.GetSubMenu(1)->GetSubMenu(5)->CheckMenuItem(IDR_OPTION_CHECK_FT_FLAG, MF_UNCHECKED | MF_BYCOMMAND);
    }
    else
    {
