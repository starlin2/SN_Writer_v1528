...
META_RESULT FeaturePhoneSN::MetaHandle_Init()
{
    ...
    {       	
        ...
        {
            sprintf(m_strMsg, "Init meta handle fail, MetaResult = %s", ResultToString(meta_result));
        }
    }
...
META_RESULT FeaturePhoneSN::EnterModemMeta()
{
	...
	{
        sprintf(m_strMsg, "Start enter meta mode...");
        UpdateMsgInfo(m_strMsg);
        pFuncName = "META_ConnectWithMultiModeTarget_r";
        ...
        {   
            ...
            MTRACE (g_hEBOOT_DEBUG, "FeaturePhoneSN::META_ConnectWithMultiModeTarget_r(): Enter meta mode successfully!");
        }
    ...
    {      
        if (g_sMetaComm.bDualTalk)
        {
            ...
            MetaResult = META_GetDynamicUSBComPortExWithFilter_r( m_hMauiMetaHandle, ms_connect_timeout, &comPort, m_pMetaStopFlag, info, 1000, asPreloaderPortFilter);
            ...
            MetaResult = META_GetDynamicUSBComPortExWithFilter_r( m_hMauiMetaHandle, ms_connect_timeout, &comPort, m_pMetaStopFlag, info, 512, asBROMPortFilter);
            if(META_SUCCESS != MetaResult)
            {               
        ...
        {
            MetaResult = META_GetDynamicUSBComPort_r (m_hMauiMetaHandle, m_tMetaReq_Ex.ms_connect_timeout, &comPort, m_pMetaStopFlag);          
            sprintf(m_strMsg, "Start enter meta mode...");
            UpdateMsgInfo(m_strMsg); 
        }
        ...
        {          
            ...
            MetaResult = META_ConnectWithMultiModeTarget_r(m_hMauiMetaHandle, &m_tMetaReq_Ex, sizeof(m_tMetaReq_Ex), m_pMetaStopFlag, &m_tMetaConnReport_Ex);               
            if (MetaResult == META_SUCCESS /*|| META_MAUI_DB_INCONSISTENT == MetaResult*/)
            {      
...
META_RESULT FeaturePhoneSN::LoadModemDatabase()
{
    return META_SUCCESS;
}
...
META_RESULT FeaturePhoneSN::LoadModemDatabase(int MDindex)
{
    ...
    if (false == g_sMetaComm.sDBFileOption.bDBInitModem_1)
    {
        ...
        if (META_SUCCESS == meta_result /*|| META_MAUI_DB_INCONSISTENT == meta_result*/)
        {
            ...
            MTRACE (g_hEBOOT_DEBUG, "FeaturePhoneSN::META_NVRAM_Init_r(): Init modem database successfully!");
        }
        else
        {
            MTRACE_ERR (g_hEBOOT_DEBUG, "FeaturePhoneSN::META_NVRAM_Init_r(): Init modem database fail, MetaResult = %s", ResultToString(meta_result));
...
META_RESULT FeaturePhoneSN::REQ_ReadModem_NVRAM_Start(WriteData_Type_e dataType, char *pOutData, unsigned short iRID)
{
    ...
    if ( META_SUCCESS != meta_result)
    {
        ...
        UpdateMsgInfo(ResultToString(meta_result));
        return meta_result;
    }
    else
    {
...
META_RESULT FeaturePhoneSN::REQ_BackupNvram2SDS_Start()
{
    ...
    sprintf ( m_strMsg, "Query target support SDS/Backup_Partition...");
    UpdateMsgInfo(m_strMsg);
    
    if(META_SUCCESS == MetaResult)
    {
        sprintf ( m_strMsg, "Trigger backup nvram to SDS/Backup_Partition start...");
        UpdateMsgInfo ( m_strMsg );
        MTRACE(g_hEBOOT_DEBUG, "FeaturePhoneSN::REQ_BackupNvram2SDS_Start(): %s", m_strMsg);
    ...
    {
        sprintf ( m_strMsg, " Target without SDS/Backup_Partition feature");
        MTRACE(g_hEBOOT_DEBUG, "FeaturePhoneSN::REQ_BackupNvram2SDS_Start(): %s", m_strMsg);
        return META_SUCCESS;;
	}
    ...
    while((fpStage != FP_END_STAGE) && (*m_pMetaStopFlag != BOOT_STOP))
    {
        ...
        case FP_BARCODE_STAGE:
            ...
            if (g_sMetaComm.sWriteOption.bWriteBarcode)
            {
                ...
                {
                    sprintf(m_strMsg, "FeaturePhoneSN::Barcode : MetaResult = %s", ResultToString(MetaResult));
            ...
            if (g_sMetaComm.sWriteOption.bWriteIMEI)
            {
                ...
                {     
                    ...
                    else
                    {
                        ...
                        sprintf(m_strMsg, "FeaturePhoneSN::IMEI[%d] : MetaResult = %s", i, ResultToString(MetaResult));
                        fpNextStage = FP_END_STAGE;
                        break;
                    }
                }
        ...
        case FP_BT_ADDRESS_STAGE:
            if (g_sMetaComm.sWriteOption.bWriteBT)
            {
                ...
                {
                    sprintf(m_strMsg, "FeaturePhoneSN::BT : MetaResult = %s", ResultToString(MetaResult));
                    fpNextStage = FP_END_STAGE;
                }
            } 
            else
        ...
        case FP_WIFI_ADDRESS_STAGE:
            if (g_sMetaComm.sWriteOption.bWriteWifi)
            {
                ...
                {
                    sprintf(m_strMsg, "FeaturePhoneSN::Wifi : MetaResult = %s", ResultToString(MetaResult));
                    fpNextStage = FP_END_STAGE;
                }
            } 
            else
...
{     
    ...
    */  
    ...
    {     
        ...
        sprintf(m_strMsg, "Start load and init database...");
        UpdateMsgInfo(m_strMsg);
        UpdateProgress(0.05);
        ...
        {       
            bAnyOperationFail = true;
            sprintf(m_strMsg, "LoadModemDatabase() : MetaResult = %s", ResultToString(MetaResult));
        ...
        {       
            bAnyOperationFail = true;
            sprintf(m_strMsg, "EnterModemMeta() : MetaResult = %s", ResultToString(MetaResult));
            goto End;
        }
        UpdateProgress(0.20);
